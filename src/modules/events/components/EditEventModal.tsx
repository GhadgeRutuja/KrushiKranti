...
import { updateEventThunk } from "../eventsSlice";
import {
  RiCloseLine,
  RiLoader4Line,
  RiImageAddLine,
...
export function EditEventModal({
  ...
  const isValidDateInput = (date: string) => /^\d{4}-\d{2}-\d{2}$/.test(date) && !Number.isNaN(new Date(date).getTime());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await dispatch(
        updateEventThunk({ id: event.id, payload }),
      );
